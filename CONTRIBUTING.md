...
- `feat`: addition, removal, or modification of a feature.
- `fix`: fixes a defect introduced by a previous `feat` commit.
- `refactor`: a modification that does not affect behavior.
- `perf`: a type of `refactor` that improves performance.
- `style`: changes code style and does not affect behavior.
- `test`: additions, removals, modifications, or fixes to tests.
- `docs`: a change that affects only documentation.
- `build`: components such as tools, dependencies, and build version.
- `ci`: components of continuous integration and continuous delivery.
- `revert`: reverts to a previous commit.
- `chore`: miscellaneous, such as updating `.gitignore`.
